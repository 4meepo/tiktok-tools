...
func (f TiktokCreatorFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.TiktokCreatorMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.TiktokCreatorMutation", m)
	}
	return f(ctx, mv)
}
